		...
		case "chegada":
			...
			if err := json.Unmarshal(msg.Data, &chegada); err != nil {
				...
				if len(reservationQueue) == 0 {
                    ...
                    if batteryToCharge < 0 { batteryToCharge = 0 } // Evitar carga negativa
...
func simulateCharging(conn net.Conn, cp *ChargingPoint, batteryToCharge int) {
    ...
    if batteryToCharge <= 0 {
        ...
        if cp.VehicleID == vehicleID { // Verifica se ainda é o mesmo veículo
            ...
            _, err := conn.Write(data)
            if err != nil { logMessage("SESSION_END", "ERROR", "Failed to send session end message: %v", err) } else { logMessage("SESSION_END", "INFO", "Session end notification sent to server for vehicle %s", vehicleID) }
	...
	if cp.VehicleID == vehicleID {
        ...
        if batteryCharged < 0 { batteryCharged = 0 } // Sanity check
